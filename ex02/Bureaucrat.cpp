...
/*   Updated: 2023/11/02 10:31:52 by axcallet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	Bureaucrat::executeForm(const AForm &form) {
	try {
		form.execute(*this);
		std::cout << _GREEN << "Bureaucrat " << this->getName() << " executed " << form.getName() << _END << std::endl;
	}
	catch (std::exception &f) {
		std::cout << _RED << "Bureaucrat " << this->_name << " couldn't execute " << form.getName() << " because " << f.what() << _END << std::endl;
	}
}
